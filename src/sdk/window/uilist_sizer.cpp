...
uiListSizer::uiListSizer()
{
	/* override property */
	SetStringProperty(PROP_CLASS,		"UIListSizer");
	m_Properties.find(PROP_BKG_COLOR)->second->m_Visiable = false;

	/* specially for UIListSizer */
	m_Properties.insert(pair<wxString, PropertyBase*>("ListSizer Property", new PropertyFold("ListSizer Property")));
	m_Properties.insert(pair<wxString, PropertyBase*>(PROP_DIRECTION, new PropertyEnum(1)));

	m_type = WINDOW_TYPE_SIZER;
	m_icon_index = ICON_CONTROL_LIST_SIZER;
}


uiListSizer::~uiListSizer(void)
{

}

wxPGChoices uiListSizer::AddEnmuChildProperty(wxString key)
{
	wxPGChoices choice;
...
UISizer* uiListSizer::guiSizer(void)
{
	UISizer *sub_sizer = create_list_sizer(GetEnumPropValue(PROP_DIRECTION));
        ...
        if (parent_sizer)
        {
            ...
            while(parent)
            {
                if (parent->GetWindowType() == WINDOW_TYPE_SUB)
                {
                    ...
                    {
                        set_container_sizer(parent->stb_window, parent_sizer);
                    }
                    break;
                }
...
TiXmlElement* uiListSizer::GetXMLElement(void)
{
...
void uiListSizer::SetProperties(const TiXmlElement* element)
{
	/* read base property */
...
void uiListSizer::OnBuildDeclarationsCode()
{

}

void uiListSizer::OnBuildCreatingCode()
{
	uiWindow::OnBuildCreatingCode();
	AddBuildingCode("\n");
}

void uiListSizer::OnBuildIdCode()
{
