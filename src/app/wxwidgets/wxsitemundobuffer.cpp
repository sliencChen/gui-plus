...
* $HeadURL: http://svn.code.sf.net/p/codeblocks/code/branches/release-17.xx/src/plugins/contrib/wxSmith/wxwidgets/wxsitemundobuffer.cpp $
*/
...
void wxsItemUndoBuffer::StoreChange(const wxString& XmlData)
{
    ...
    //wxLogMessage("m_CurrentPos = %d, GetCount = %d", m_CurrentPos, GetCount());
}

const wxString& wxsItemUndoBuffer::Undo()
{
    if ( m_CurrentPos == 0 ) return Empty;
    //wxLogMessage("Undo m_CurrentPos = %d, GetCount = %d", m_CurrentPos, GetCount());
...
const wxString& wxsItemUndoBuffer::Redo()
{
    ...
    //wxLogMessage("Redo m_CurrentPos = %d, GetCount = %d", m_CurrentPos, GetCount());
    return m_Enteries[++m_CurrentPos];
}
